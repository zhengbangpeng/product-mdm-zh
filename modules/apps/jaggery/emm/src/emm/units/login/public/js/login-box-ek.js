    ...
    $('form').each(function () {
        $(this).find('input').keypress(function (e) {
            if (e.which == 10 || e.which == 13) {
                ...
                if (!username) {
                    $('.wr-validation-summary p').text("用户名不能为空");
                    $('.wr-validation-summary').removeClass("hidden");
                } else if (!password) {
                    $('.wr-validation-summary p').text("密码不能为空");
                    $('.wr-validation-summary').removeClass("hidden");
                } else {
...
$(document).ready(function () {
    ...
    if (currentHash == "#authentication-failed") {
        $('.wr-validation-summary p').text("请输入正确的用户名或密码");
        $('.wr-validation-summary').removeClass("hidden");
    } else if (currentHash == "#authorization-failed") {
        $('.wr-validation-summary p').text("对不起，您的权限不足。");
        $('.wr-validation-summary').removeClass("hidden");
    } else if (currentHash == "#error") {
        $('.wr-validation-summary p').text("登录失败");
        $('.wr-validation-summary').removeClass("hidden");
    }
