...
$(document).ready(function () {
            ...
            invokerUtil.post(
                ...
                function (data) {
                    ...
                    if (data.responseCode == responseCodes["CREATED"]) {
                        ...
                        $(errorMsg).text("操作未被允许");
                        $(errorMsgWrapper).removeClass("hidden");
                    } else {
                        $(errorMsg).text("发生未知错误");
                        $(errorMsgWrapper).removeClass("hidden");
                    }
    ...
    $("button#save-general-btn").click(function () {
        ...
        if (!notifierFrequency) {
            $(errorMsg).text("监控周期不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!isPositiveInteger(notifierFrequency)) {
            $(errorMsg).text("监控周期格式错误");
            $(errorMsgWrapper).removeClass("hidden");
        } else {
    ...
    $("button#save-ios-btn").click(function () {
        ...
        if (!configCountry) {
            $(errorMsg).text("SCEP country 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!configState) {
            $(errorMsg).text("SCEP state 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!configLocality) {
            $(errorMsg).text("SCEP locality 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!configOrganization) {
            $(errorMsg).text("SCEP organization 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!configOrganizationUnit) {
            $(errorMsg).text("SCEP organization unit 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!MDMCertPassword) {
            $(errorMsg).text("MDM 证书 密码 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!MDMCertTopicID) {
            $(errorMsg).text("MDM 证书 topic ID 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!APNSCertPassword) {
            $(errorMsg).text("APNS 证书 密码 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (notSupportedError) {
            $(errorMsg).text("浏览器不支持该APIs文件");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (invalidFormatMDMCert) {
            $(errorMsg).text("MDM 证书 必须为 pfx 格式.");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (base64MDMCert == '') {
            $(errorMsg).text("MDM 证书 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (invalidFormatAPNSCert) {
            $(errorMsg).text("APNS 证书 必须为 pfx 格式.");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (base64APNSCert == '') {
            $(errorMsg).text("APNS 证书 不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else if (!configOrgDisplayName) {
            $(errorMsg).text("组织名不能为空");
            $(errorMsgWrapper).removeClass("hidden");
        } else {
            var addConfigFormData = {};
            var configList = new Array();
    ...
    $(fileInputWindowsMDMCert).change(function () {
        ...
        if (!window.File || !window.FileReader || !window.FileList || !window.Blob) {
            $(errorMsgWindows).text("浏览器不支持该APIs文件");
    ...
    $("button#save-windows-btn").click(function () {
        ...
        if (!notifierFrequency) {
            $(errorMsgWindows).text("轮询周期不能为空");
            $(errorMsgWrapperWindows).removeClass("hidden");
        } else if (!windowsLicense) {
            $(errorMsgWindows).text("协议不能为空");
            $(errorMsgWrapperWindows).removeClass("hidden");
        } else if (!$.isNumeric(notifierFrequency)) {
            $(errorMsgWindows).text("轮询周期格式错误");
            $(errorMsgWrapperWindows).removeClass("hidden");
        } else {
            var addConfigFormData = {};
            var configList = new Array();
