...
var config = function () {
    ...
    //print(require("/modules/user.js").userModule);
    //print("<br>");
    if (!conf) {
...
var language = function () {
    var lang = application.get("PINCH_LANGUAGE");
    if (!lang) {
        var pinch = require('/modules/pinch.min.js').pinch;
        var language = require('/config/language.json');
        pinch(language, /^/, function (path, key, value) {
            return  value;
        });
    application.put("PINCH_LANGUAGE", language);
    lang = language;
    }
    return lang;
